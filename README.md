...
accept either the short id or the full id. `PROJECT` and `SOURCE` only
appear when the corpus has more than one distinct value; `TAGS` only
appears when at least one plan has tags; `CREATED` only appears when at
least one plan has the frontmatter field. Columns are content-width --
...
is the only column that shrinks, toward its floor; `PLAN` is never
truncated, only dropped, along with the other columns, in order
