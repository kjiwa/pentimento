...
`list` is one line per plan, id-led: `PLAN` is the id every other command
takes (`show`, `set`, `check`'s `plan_id`). `PROJECT` and `SOURCE` only
appear when the corpus has more than one distinct value; `TAGS` only
appears when at least one plan has tags. Every command guarantees its
output fits `style.terminal_width()` -- when a line would overflow, columns
drop in order (`TAGS`, `SOURCE`, `PROJECT`, then `PLAN` shrinks toward a
12-column floor, truncating with an ellipsis, then `INTENT`, then `STATUS`)
and `PLAN` absorbs any leftover width so `AGE` stays flush right.

<!-- sample:list -->
```
STATUS       INTENT     PROJECT   PLAN                       TAGS            AGE
superseded   abandoned  platform  docs-style-guide                            6w
complete     abandoned  platform  api-auth-redesign          auth, security   5w
complete     someday    billing   billing-dunning-copy       billing          3w
partial      active     platform  api-auth-rollout           auth, security   2w
not-started  queued     platform  api-auth-cleanup           auth, security   2w
unknown      unset      billing   billing-invoice-retry      billing          1w
not-started  active     billing   search-relevance-tuning    search           5d
unknown      unset                onboarding-checklist                        1d
...
│    billing-dunning-copy  complete  someday  3w  [billing]
├─ Retry failed invoice charges (parent elided: no-such-plan)
│    billing-invoice-retry  unknown  unset  1w  [billing]
└─ Tune search relevance
     search-relevance-tuning  not-started  active  5d  [search]

platform
├─ Write a docs style guide
│    docs-style-guide  superseded  abandoned  6w
└─ Redesign the auth API
     api-auth-redesign  complete  abandoned  5w  [auth, security]
   └─ Roll out the new auth API
        api-auth-rollout  partial  active  2w  [auth, security]
      └─ Remove the old auth API
           api-auth-cleanup  not-started  queued  2w  [auth, security]
...
<!-- sample:check -->
```
dangling-parent billing-invoice-retry: parent 'no-such-plan' does not resolve t…
